...
export async function startServer(): Promise<StartedServer> {
  ...
  logger.info({ authProvider }, "Auth provider");

  let authReady = config.deploymentMode === "local_trusted";
  let betterAuthHandler: RequestHandler | undefined;
  // Typed as AuthSessionResult to accept both BetterAuth and Neon Auth resolvers
  let resolveSession: ((req: ExpressRequest) => Promise<AuthSessionResult | null>) | undefined;
  ...
  if (config.deploymentMode === "authenticated") {
    if (authProvider === "neon") {
      ...
      resolveSession = resolveNeonAuthSession;
