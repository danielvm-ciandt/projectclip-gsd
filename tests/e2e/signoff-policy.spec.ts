...
test.describe("Signoff execution policy", () => {
  ...
  test.beforeEach(() => {
    issueCountBefore = ctx?.issueIds.length ?? 0;
  });

  test.afterEach(async () => {
    if (!ctx) return;
    const issuesCreatedInTest = ctx.issueIds.slice(issueCountBefore);
    for (const issueId of issuesCreatedInTest) {
      await ctx.boardRequest
        .patch(`${BASE_URL}/api/issues/${issueId}`, {
          data: { status: "cancelled", comment: "afterEach run-lock cleanup" },
        })
        .catch(() => {});
    }
  });
